...
def assistant_speech_output(user_input: str, result_character: str, possible_actions: list[str]) -> str:
    if (result_character == "?"):
        return "I'm unable to run that action."
    if (result_character == "*"):
        return "Please request an action for me to run."
...
Respond to the user's request in as few words as possible.
    """
...
def text_to_action(input_text: str, possible_actions: list[str]) -> str:
    ...
    logprob_text += "\nIf it doesn't seem like the user requested an action, return the * character."
